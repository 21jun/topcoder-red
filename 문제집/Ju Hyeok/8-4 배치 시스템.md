...
0부터 시작하는 n개의 작업 번호를 처리 순서로 다시 배열하고 int 자료형의 배열로 리턴해주세요.</br> 여러 가지 결과가 나온다면 사전 순서로 정맇서 가장 앞의 결과를 리턴해주세요.</br> 이름의 대소문자를 구분합니다.</br>
...
public:
	vector <int> schedule(vector <int> duration, vector <string> user)
	{
		...
		vector<bool> done(N);	// 순서표에 추가 됬는지 검사할 때 이용하는 vector
