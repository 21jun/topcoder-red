...
```
여러분은 최근 회사에서 승진과 함께 보너스를 받았습니다. 받은 보너스를 유용하게 활용하고자 주식 시장에 투자하기로 했습니다. 그리고 최근 시장의 움직임을 알고 싶어서 사고 싶다고 생각했던 주식의 이력 데이터를 모았습니다.
실험으로 선택한 주식이 얻은 성과를 평가하고 싶습니다. 그래서 개시 시간에 초기 투자로 int initialInvestment 달러를 사용하고 이후에는 매 월 int monthlyContribution 달러만큼을 사용해서 어느 정도 이익을 내는지 알아보기로 했습니다. 매월의 시작에 주식을 구매(단주 포함)하고 최후에 가지고 있는 모든 주식을 판매합니다. 기간 중에는 가진 주식을 팔지 않습니다.
여러분에게는 String[] stockPrices가 주어집니다. 이 stockPrices에 있는 각 요소에는 매월 초의 각 주식 값이 있습니다. stockPrices의 각 요소는 "stock_0 stock_1 ..." 문자열의 형태를 취합니다. 각 stock_i는 0부터 시작하지 않은 양의 정수입니다. stockPrices의 1번째 요소는 주식의 최초 가격입니다. 2번째 요소는 다음 달 초의 가격입니다. 이 달은 투자한 이후 맞이하는 첫 달 입니다. 3번째 요소 이후도 마찬가지입니다.
